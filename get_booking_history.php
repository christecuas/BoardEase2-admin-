...
try {
    ...
    // For owner: Get completed/cancelled bookings (history) for boarding houses owned by this user
    if ($userType === 'owner') {
        // Verify owner exists
        ...
        // Get completed/cancelled bookings (history) for boarding houses owned by this owner
        ...
        echo json_encode(array(
            'success' => true,
            'data' => array(
                'booking_history' => $formattedBookings
            )
        ));
    } else {
        echo json_encode(array(
    ...
    error_log("Database error in get_booking_history.php: " . $e->getMessage());
    echo json_encode(array(
        'success' => false,
